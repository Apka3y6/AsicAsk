...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if (indexPath != nil) {
        MMDevice *selectedDevice = [self.presenter.connectedDevices objectAtIndex:indexPath.row];
        NSLog(@"Selected IP Address: %@", selectedDevice.ipAddress); // Print the IP address to console
        
        if ([segue.identifier isEqualToString:@"hostCellSegue"]) {
            TCPVC *tcpvc = [segue destinationViewController];
            tcpvc.selectedIpAddress = selectedDevice.ipAddress; // Pass ipAddress to TCPVC
        }
    }
}
